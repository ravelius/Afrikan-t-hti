...
const MANNER_ZOOM = 2.3;        // vanha kiinteä kerroin; kts. ZOOMI_LEVEYDET
 ...
 */
const ZOOMI_LEVEYDET = [667, 435, 294, 200, 132, 88];
// Mantereelle saavuttaessa zoomataan tähän portaaseen (vastaa vanhaa
// kerrointa 2.3 tuhannen yksikön laudalla).
const ZOOMI_OLETUS = 2;
...
export class UI {
  ...
  zoomiTasot() {
    ...
    const tasot = [1];
    for (const nakyva of ZOOMI_LEVEYDET) {
      const kerroin = leveys / nakyva;
      if (kerroin > 1.05) tasot.push(kerroin);
    }
    return tasot;
  }
  ...
  get zoomiKerroin() {
    ...
    return tasot[this.zoomiIndeksi] ?? tasot[ZOOMI_OLETUS] ?? MANNER_ZOOM;
  }
