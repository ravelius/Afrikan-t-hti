...
// Näkymän zoomaus lennon reitille ja siitä takaisin.
const ZOOM_MS = 700;
// Kuinka pitkään repliikki saa levätä ruudussa kirjoittumisen jälkeen.
const FLIGHT_LINE_HOLD_MS = 1600;
...
export class UI {
  ...
  renderTravelChoice(modes) {
    ...
    for (const link of gateways) {
      ...
      gwBtn.addEventListener('click', async () => {
        ...
        // Reunan yli katoavaa lentoa ei zoomata: kohde ei ole tällä kartalla.
        await this.animateFlight(
          lahto, this.mapEdgeToward(lahto),
          game.flightLine(link.city, packById(link.pack)), { zoom: false },
        );
  ...
  async doPickStart(city) {
    ...
    // Pelin avaus on se filmihetki: avausteksti häipyy, näkymä zoomaa
    // Lontoon ja kohteen ympärille ja kone lentää ennen kuin mantereen
    // kartta aukeaa. Takaisin ei zoomata — lauta vaihtuu heti perään.
  ...
  /**
   * Zoomaa näkymän kahden pisteen ympärille lennon ajaksi. Palauttaa
   * lähtönäkymän viewBox-merkkijonon, jotta sen voi ajaa takaisin.
   */
  async zoomToFlight(a, b) {
    const alkuperainen = this.svg.getAttribute('viewBox');
    const pane = this.svg.parentElement;
    if (!pane.clientWidth || !pane.clientHeight) return alkuperainen;

    // Laatikko pisteiden ympärille: reilu marginaali, eikä ikinä niin
    // tiukka etteivät kaupunkien nimet ja kaari mahtuisi kuvaan.
    const PAD = 130;
    const MIN_SPAN = 420;
    let x0 = Math.min(a.x, b.x) - PAD;
    let y0 = Math.min(a.y, b.y) - PAD;
    let x1 = Math.max(a.x, b.x) + PAD;
    let y1 = Math.max(a.y, b.y) + PAD;
    ...
    const scale = Math.min(pane.clientWidth / (x1 - x0), pane.clientHeight / (y1 - y0));
    const vw = pane.clientWidth / scale;
    const vh = pane.clientHeight / scale;
    const kohde = [
      (x0 + x1) / 2 - vw / 2, (y0 + y1) / 2 - vh / 2, vw, vh,
    ];
    await this.animateViewBox(kohde, ZOOM_MS);
    return alkuperainen;
  }

  /** Liu'uttaa viewBoxin nykyisestä kohteeseen pehmeällä käyrällä. */
  async animateViewBox(kohde, ms) {
    const lahto = (this.svg.getAttribute('viewBox') ?? '0 0 1000 1000')
      .split(/\s+/).map(Number);
    await new Promise((resolve) => {
      const alku = performance.now();
      const askel = (nyt) => {
        const t = Math.min(1, (nyt - alku) / ms);
        const e = t < 0.5 ? 2 * t * t : 1 - (-2 * t + 2) ** 2 / 2;
        const arvot = lahto.map((v, i) => v + (kohde[i] - v) * e);
        this.svg.setAttribute('viewBox', arvot.join(' '));
   ...
   *
   * Näkymä zoomaa lennon ajaksi lähelle lähtöä ja kohdetta (zoom: false
   * ohittaa, esim. porttilennot kartan reunan yli). zoomBack palauttaa
   * näkymän lennon jälkeen — pois silloin, kun lauta vaihtuu heti perään.
   * Repliikki pidetään ruudussa, kunnes se on ehditty kirjoittaa ja lukea.
   *
   ...
   */
  async animateFlight(a, b, line = null, { zoom = true, zoomBack = true } = {}) {
    if (this.reducedMotion || !a || !b) return;
    this.flightLayer.textContent = '';
    const palautus = zoom ? await this.zoomToFlight(a, b) : null;
    ...
    // Kone jää hetkeksi kohteeseen ja repliikki saa kirjoittua loppuun ja
    // tulla luetuksi — omistajan palaute: teksti hävisi liian nopeasti.
    if (line) {
      const odota = Math.max(0, (this.flightLineReadyAt ?? 0) - performance.now())
        + FLIGHT_LINE_HOLD_MS;
      await new Promise((resolve) => { setTimeout(resolve, odota); });
    }

    if (palautus && zoomBack) await this.animateViewBox(palautus.split(/\s+/).map(Number), ZOOM_MS);
    this.flightLayer.textContent = '';
    this.hideFlightLine();
  }
  ...
  showFlightLine(line) {
    ...
    // Arvio kirjoituskoneen valmistumishetkestä pitoa varten.
    this.flightLineReadyAt = performance.now() + String(line).split(' ').length * TYPE_MS;
    // Sijainti lasketaan NÄKYVÄSTÄ viewBoxista, ei koko kartasta: lennon
    // ajaksi näkymä on zoomattu lähelle reittiä, ja koko karttaan sidottu
    // rivi jäisi kuvan ulkopuolelle.
    const vb = this.svg.viewBox.baseVal;
