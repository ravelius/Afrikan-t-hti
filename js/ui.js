...
export class UI {
  ...
  zoomToFlight(a, b) {
    ...
    // Marginaali kasvaa matkan mukana: reitti vie aina noin 60 % ruudusta.
    const dist = Math.hypot(b.x - a.x, b.y - a.y);
    const PAD = Math.max(60, dist * 0.3);
    const MIN_SPAN = 240;
  ...
  async animateFlight(a, b, line = null, { zoom = true, zoomBack = true } = {}) {
    ...
    const palautus = zoom ? this.zoomToFlight(a, b) : null;
    ...
    if (palautus && zoomBack) this.svg.setAttribute('viewBox', palautus);
    this.flightLayer.textContent = '';
    this.hideFlightLine();
  }
